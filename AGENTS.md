# Automatic per-repo GitHub account selection for the `gh` CLI (`gh-account-autoswitch`) — factory product contract

This repository is one product built by the Param Factory. It ships at
`https://gh-account-autoswitch.sociobot.in`. Artifact class: `cli`.
